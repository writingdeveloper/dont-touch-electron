...
모든 영상 처리는 기기에서 로컬로 수행되며, 카메라 영상은 절대 기기를 벗어나지 않습니다. 앱 개선을 위해 익명의 사용 통계(개인정보·이미지 제외)만 수집됩니다.

## 주요 기능

- MediaPipe 기반 실시간 얼굴/손 감지
- 감지 영역 설정 (두피, 눈썹, 눈, 볼 등)
- 전체 화면 알림 + 맞춤 설정 가능한 알림음 (내장 톤, 다국어 음성, 사용자 오디오)
- 일일 통계 및 스트릭 추적
- 내장 호흡 명상
- 시스템 트레이 지원
- 다국어 UI
...
- 모든 영상·이미지 처리는 로컬에서 수행 — 카메라 프레임이 기기를 벗어나지 않음
- 앱 개선을 위해 익명의 사용 통계(예: 앱 실행, 감지 시작/중지)만 수집 — 개인정보·이미지·영상 미수집
- 분석 데이터는 GDPR, CCPA, PIPEDA에 부합하게 처리
